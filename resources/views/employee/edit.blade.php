...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card">
        ...
        <div class="card-body">
        @foreach ($edit as $ep)
            <form action="{{ route('editep.update', ['employee' => $ep->id]) }}" method="POST">
                @csrf
                @method('PUT') 
                <div class="mb-3">
                    <label for="name" class="form-label">Name</label>
                    <input type="text" class="form-control" id="name" name="name" value="{{ $ep->name }}" placeholder="Enter employee's name" required>
                </div>

                <div class="mb-3">
                    <label for="email" class="form-label">Email</label>
                    <input type="email" class="form-control" id="email" name="email" value="{{ $ep->email }}" placeholder="Enter employee's email" required>
                </div>

                <div class="mb-3">
                    <label for="phone" class="form-label">Phone</label>
                    <input type="text" class="form-control" id="phone" name="phone" value="{{ $ep->phone }}" placeholder="Enter employee's phone number" required>
                </div>

                <div class="mb-3">
                    <label for="address" class="form-label">Address</label>
                    <input type="text" class="form-control" id="address" name="address" value="{{ $ep->address }}" placeholder="Enter address" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="password" class="form-control" id="password" name="pass" placeholder="Enter new password (optional)">
                </div>
